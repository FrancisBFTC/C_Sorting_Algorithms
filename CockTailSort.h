...
// Cocktail shaker sort,[1] também conhecido como bubble sort bidirecional,[2] cocktail sort, 
// shaker sort (o qual também pode se referir a uma variação do insertion sort), ripple sort, shuffle sort,[3] ou shuttle sort, 
...
// A complexidade do Cocktail shaker sort em notação big-O é O ( n 2 ) {\displaystyle O(n^{2})} O(n^{2}) para o pior caso e caso médio,
// mas tende a se aproximar de O ( n ) {\displaystyle O(n)} O(n) se a lista se encontra parcialmente ordenada antes da execução do algoritmo. 
